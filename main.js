/*
* Я загружаю города из файла. Этот подход не является оптимальным.
* Лучшим подходом с моей точки зрения является хранение базы городов в базе и взаимодействие с базой через API запросы к бекенду.
Таким образом не придется перегружать фронтенд излишними данными.
Фронтенд будет отсылать запрос на бекенд для создания сессии и потом на проверку города,введенного пользователем, а бекенд будет возвращать true или false.
В случае, если такой город уже был, бекенд будет кешировать(сохранять в коллекцию) у себя этот город до окончания сессии.
Во время хода компьютера также будет отсылаться запрос на бекенд, который будет смотреть, какие города уже были выданы и запрашивать из базы те, что еще не были введены пользователем или выданы компьтером.
Перед возвратом на фронтенд значение добавляются в кеш(коллекцию) не бекенде.
Через определенное время сессия будет експайриться, и кеш будет очищаться.
*
* */
